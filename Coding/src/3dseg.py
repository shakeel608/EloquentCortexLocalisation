...
def main(tempdir):
    ...
    train_files = [{"img": img, "seg": seg} for img, seg in zip(images[:120], segs[:120])]
    val_files = [{"img": img, "seg": seg} for img, seg in zip(images[-120:], segs[-120:])]
    ...
    check_loader = DataLoader(check_ds, batch_size=batch_size, num_workers=1, collate_fn=list_data_collate)
    ...
    val_loader = DataLoader(val_ds, batch_size=batch_size, num_workers=1, collate_fn=list_data_collate)
    ...
    wandb.watch(model, log_freq=5, log_graph=True)
    ...
    for epoch in range(100):
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                wandb.log({"Val Dice Metric": metric}, step=epoch+1)
